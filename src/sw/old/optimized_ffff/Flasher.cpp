...
void RxStart()
{
    ...
    printf("RX:(%08d) ", g_RxSize); for(uint32_t i = 0 ; i< g_RxSize ; printf("%02X", buf[i++])); printf("\n");
...
void TxStart()
{
    ...
    printf("TX:(%08d) ", g_TxSize); for(uint32_t i = 0 ; i< g_TxSize ; printf("%02X", (unsigned char)g_TxBuffer[i++])); printf("\n");
...
void AddCMD(uint8_t Cmd, uint8_t ParamSize, uint32_t Parameter)
{
    if((g_TxSize + ParamSize + 1) > TX_THRESHOLD)
        RxStart();

    g_TxBuffer[g_TxSize++] = Cmd;
    while(ParamSize--) {
        g_TxBuffer[g_TxSize++] = (uint8_t) (Parameter & 0xFF);
        Parameter >>= 8;
    }
    if(g_TxSize == TX_THRESHOLD)
        RxStart();
...
void SetTrigger(uint16_t wMask, uint16_t wValue)
{
        if(!wMask)
                return;
        AddCMD(0x08, 4, ((uint32_t) wMask << 16) | wValue);
...
void NOR_Erase(uint32_t addr)
{
    ...
    WriteToAddress(0x555 | addr, 0xAA);
    WriteToAddress(0x2AA | addr, 0x55);
    WriteToAddress(0x555 | addr, 0x80);
    WriteToAddress(0x555 | addr, 0xAA);
    WriteToAddress(0x2AA | addr, 0x55);
