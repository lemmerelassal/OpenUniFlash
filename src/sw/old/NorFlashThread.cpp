...
void NorFlashThread::Operation()
{
    ...
    for (ib = blockMap.begin(); ib != ibe && !canceled; ib++) {
        ...
        if (!(*ib).second.erased) {
            ...
            ::NOR_Erase(addr);
            ::Delay_us(50);
//            QByteArray erasebuf(len, 0xFF);
//            QByteArray verasebuf(len, 0xFF);
//                ::NOR_Read(verasebuf.data(), addr, len);
//                if(erasebuf != verasebuf)
//                    statusTextAdded(tr("Erase failed."));
        }
    ...
    statusTextAdded(tr("Resetting NOR before verification."));
    ::NOR_Reset();
    ...
    for (ib = blockMap.begin(); ib != ibe && !canceled; ib++) {
        ...
        QByteArray verifyBuf2(len, 0xFF);

        ::NOR_Read(verifyBuf.data(), addr, len);



        if (((*ib).second.file != verifyBuf) && ((*ib).second.file != verifyBuf)) {
            statusTextAdded(tr("Verification of address 0x%1 failed").arg(addr, 0, 16));
            ::NOR_Read(verifyBuf2.data(), addr, len);
            if (verifyBuf != verifyBuf2)
                statusTextAdded(tr("Inconsistency while reading (bad connection or programming still in execution"));
